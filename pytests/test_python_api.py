...
def _segment_test_writer(
    ...
    *,
    ...
    stream = master.get_stream(stream_name)
    return zippy._internal._SegmentTestWriter(
        stream_name,
        schema,
...
def test_session_descriptor_publisher_prefers_envelope_publish_fast_path() -> None:
    class FakeMaster:
        def __init__(self) -> None:
            self.envelopes: list[tuple[str, bytes, dict[str, object] | None]] = []
            self.descriptors: list[tuple[str, dict[str, object]]] = []
        ...
        def publish_segment_descriptor_envelope(
            ...
            self.envelopes.append((stream_name, bytes(payload), metadata))
    ...
    master = FakeMaster()
    session = zippy.Session(name="fast_descriptor_session", master=master)

    publish = session._descriptor_publisher(
        "ticks",
        descriptor_metadata={"zippy_table_kind": "append_table"},
    )
    publish(b'{"segment_id":1,"generation":0}')

    deadline = time.time() + 1.0
    while not master.envelopes and time.time() < deadline:
        time.sleep(0.01)

    assert master.envelopes == [
        (
            "ticks",
...
def test_session_publish_append_table_sets_append_table_kind(monkeypatch) -> None:
    ...
    with pytest.warns(DeprecationWarning, match="append_table"):
...
def test_session_append_table_publish_sets_append_table_kind(monkeypatch) -> None:
    schema = pa.schema(
        [
            ("instrument_id", pa.string()),
            ("last_price", pa.float64()),
    ...
    master = FakeMaster()
    session = zippy.Session(name="factor_session", master=master)
    returned = session.engine(FakeEngine()).append_table("factor_events").publish()
...
def test_session_append_table_publish_uses_stream_status_for_writer_epoch(
    monkeypatch,
) -> None:
    ...
    class FakeMaster:
        ...
        def get_stream_status(self, stream_name: str) -> dict[str, object]:
            assert stream_name == "factor_events"
            return {"writer_epoch": 7}

        def get_stream(self, stream_name: str) -> dict[str, object]:
            raise AssertionError("writer_epoch lookup should not fetch full stream metadata")
...
def test_session_append_table_publish_error_mentions_current_api() -> None:
    session = zippy.Session(name="factor_session", master=zippy.MasterClient())

    with pytest.raises(RuntimeError, match=r"append_table\(\.\.\.\)\.publish\(\) requires"):
        session.append_table("factor_events").publish()


def test_session_source_key_value_table_publish_error_mentions_current_api() -> None:
    session = zippy.Session(name="factor_session", master=zippy.MasterClient())

    with pytest.raises(
        ValueError,
        match=r"source\(\.\.\.\)\.key_value_table\(\.\.\.\)\.publish\(\) table name",
    ):
        session.source("factor_events").key_value_table("", by=["instrument_id"]).publish(
            start_from="tail"
        )
...
def test_session_publish_key_value_table_sets_key_value_table_kind(monkeypatch) -> None:
    ...
    master = FakeMaster()
    session = zippy.Session(name="factor_session", master=master)
    with pytest.warns(DeprecationWarning, match="key_value_table"):
...
def test_session_key_value_table_publish_sets_key_value_table_kind(monkeypatch) -> None:
    schema = pa.schema(
        [
            ("instrument_id", pa.string()),
            ("last_price", pa.float64()),
    ...
    returned = (
        ...
        .key_value_table("latest_factors", by=["instrument_id"])
        .publish()
    )
...
def test_session_publish_append_table_warns_deprecated(monkeypatch) -> None:
    ...
    session = zippy.Session(name="factor_session", master=FakeMaster()).engine(FakeEngine())

    with pytest.warns(DeprecationWarning, match="append_table"):
        assert session.publish_append_table("factor_events") is session
...
def test_session_source_key_value_table_publish_uses_stream_status_for_source_kind(
    monkeypatch,
) -> None:
    schema = pa.schema(
        [
    ...
    class FakeMaster:
        ...
        def get_stream(self, stream_name: str) -> dict[str, object]:
            raise AssertionError("source kind check should not fetch full stream metadata")
    ...
    zippy.Session(name="source_session", master=FakeMaster()).source(
        "factor_events"
    ).key_value_table("latest_factors", by=["instrument_id"]).publish(start_from="tail")


def test_session_source_key_value_table_publish_tail_registers_materializer(monkeypatch) -> None:
    schema = pa.schema(
        [
            ("instrument_id", pa.string()),
            ("last_price", pa.float64()),
...
def test_session_engine_rejects_explicit_target() -> None:
    class CapturingEngine:
        def __init__(self, *, name, source, master=None, target=None):
            self._status = "created"
...
def test_reactive_factor_uses_its_own_id_column() -> None:
    schema = pa.schema(
        [
            ("symbol", pa.string()),
            ("venue", pa.string()),
    ...
    try:
        ...
        assert received.column_names == ["symbol", "venue", "price", "venue_ret"]
        assert received.column("venue_ret").to_pylist() == [None, None, 1.0]
    finally:
        engine.stop()
        if subscriber is not None:
            subscriber.close()
...
def test_python_package_exposes_git_derived_version() -> None:
    ...
    expected = f"{base_version}.dev{commit_count}+g{short_sha}"
...
def test_parquet_sink_constructor_is_rejected(tmp_path: Path) -> None:
    with pytest.raises(TypeError, match="ParquetSink is no longer supported"):
...
def test_reactive_engine_accepts_rollback_state_failure_policy() -> None:
    schema = pa.schema([("symbol", pa.string()), ("price", pa.float64())])
    ...
    engine = zippy.ReactiveStateEngine(
        ...
        input_schema=schema,
        id_column="symbol",
        factors=[zippy.TS_RETURN(column="price", period=1, output="ret_1")],
        target=zippy.NullPublisher(),
        state_failure_policy="rollback",
    )

    assert engine.config()["state_failure_policy"] == "rollback"


def test_reactive_engine_rejects_invalid_state_failure_policy() -> None:
    schema = pa.schema([("symbol", pa.string()), ("price", pa.float64())])

    with pytest.raises(ValueError, match="state_failure_policy"):
        zippy.ReactiveStateEngine(
            name="tick_factors",
            input_schema=schema,
            id_column="symbol",
            factors=[zippy.TS_RETURN(column="price", period=1, output="ret_1")],
            target=zippy.NullPublisher(),
            state_failure_policy="checkpoint",
        )
...
@pytest.mark.parametrize("id_filter", [[""], ["  "]])
def test_reactive_engine_rejects_blank_id_filter_values(id_filter) -> None:
    schema = pa.schema(
        [
            ("symbol", pa.string()),
            ("price", pa.float64()),
    ...
    with pytest.raises(ValueError, match="id_filter contains empty value"):
        zippy.ReactiveStateEngine(
            name="reactive_filter",
            input_schema=schema,
            id_column="symbol",
...
def test_timeseries_engine_config_and_lifecycle_roundtrip() -> None:
    input_schema = pa.schema(
        [
            ("symbol", pa.string()),
            ("dt", pa.timestamp("ns", tz="UTC")),
...
@pytest.mark.parametrize("id_filter", [[""], ["  "]])
def test_timeseries_engine_rejects_blank_id_filter_values(id_filter) -> None:
    schema = pa.schema(
        [
            ("symbol", pa.string()),
            ("dt", pa.timestamp("ns", tz="UTC")),
...
def test_reactive_engine_legacy_parquet_sink_is_rejected() -> None:
    schema = pa.schema(
        [
            ("symbol", pa.string()),
            ("price", pa.float64()),
...
def test_stream_table_engine_supports_source_and_target_config() -> None:
    schema = pa.schema(
        [
            ("symbol", pa.string()),
            ("price", pa.float64()),
...
def test_stream_table_engine_rejects_legacy_parquet_sink() -> None:
    schema = pa.schema(
        [
            ("symbol", pa.string()),
            ("price", pa.float64()),
...
def test_stream_table_engine_passthrough_publishes_remote_stream() -> None:
    schema = pa.schema(
        [
            ("symbol", pa.string()),
            ("dt", pa.timestamp("ns", tz="UTC")),
...
def test_reactive_engine_can_consume_segment_stream_source(tmp_path: Path) -> None:
    ...
    writer = _segment_test_writer(writer_master, "ticks", schema, row_capacity=16)
...
def test_cross_sectional_engine_config_and_lifecycle() -> None:
    input_schema = pa.schema(
        [
            ("symbol", pa.string()),
            ("dt", pa.timestamp("ns", tz="UTC")),
...
def test_legacy_bus_ring_api_is_not_exported() -> None:
    assert not hasattr(zippy, "read_from")
    assert not hasattr(zippy, "BusReader")
    assert not hasattr(zippy, "BusWriter")
    assert "read_from" not in zippy.__all__
    assert "BusReader" not in zippy.__all__
    assert "BusWriter" not in zippy.__all__
    assert not hasattr(zippy.MasterClient, "write_to")
    assert not hasattr(zippy.MasterClient, "read_from")
    internal_stub = Path(zippy.__file__).with_name("_internal.pyi").read_text()
    assert "class BusWriter" not in internal_stub
...
def test_master_client_lists_streams(tmp_path: Path) -> None:
    ...
    assert streams[0]["write_seq"] == 0
    assert streams[0]["writer_process_id"] is None
    assert streams[0]["writer_epoch"] == 0
...
def test_master_client_gets_stream(tmp_path: Path) -> None:
    ...
    assert stream["write_seq"] == 0
    assert stream["writer_process_id"] is None
    assert stream["writer_epoch"] == 0
...
def test_ops_compact_table_sorts_compacted_partition_by_dt(
    tmp_path: Path,
) -> None:
    schema = pa.schema(
        [
    ...
    persisted_files = [
        {
            ...
            "persist_data_root": str(tmp_path / "persisted"),
            "row_count": 2,
            "source_segment_id": 1,
            "source_generation": 0,
    ...
    compacted = master.replaced_files[0]
    compacted_table = pq.read_table(compacted["file_path"], partitioning=None)
    dt_values = pc.cast(compacted_table.column("dt"), pa.int64()).to_pylist()
    assert dt_values == [1, 2, 3]
    assert compacted["stats"]["dt"]["min"] == 1
    assert compacted["stats"]["dt"]["max"] == 3
    assert compacted["min_event_ts"] == 1
    assert compacted["max_event_ts"] == 3
        ...
        def get_stream(self, table_name: str) -> dict[str, object]:
            return {
                ...
                "active_segment_preflight": {
                    "status": "error",
                    "kind": "active_segment_unreadable",
                    "message": "failed to open active segment shm_os_id=[file:/tmp/missing]",
                },
                "persist_events": [],
            }

    health = zippy.ops.table_health("ghost_ticks", master=FakeMaster())

    assert health["status"] == "error"
    assert health["alert_count"] == 1
    assert health["alerts"][0]["severity"] == "error"
    assert health["alerts"][0]["kind"] == "active_segment_unreadable"
    assert "failed to open active segment" in health["alerts"][0]["message"]


def test_table_info_exposes_segment_row_capacity_separately_from_bus_buffer() -> None:
    class FakeMaster:
        def get_stream(self, table_name: str) -> dict[str, object]:
            return {
                "stream_name": table_name,
                ...
                "buffer_size": 64,
                "frame_size": 4096,
                "segment_row_capacity": 65536,
                "descriptor_generation": 1,
                "active_segment_descriptor": {"row_capacity": 65536},
                "active_segment_preflight": {
                    "status": "ok",
    ...
    info = zippy.ops.table_info("ticks", master=FakeMaster())

    assert info["buffer_size"] == 64
    assert info["segment_row_capacity"] == 65536
    assert info["active_segment_preflight"]["row_capacity"] == 65536


def test_wait_for_table_ready_times_out_on_failed_active_segment_preflight() -> None:
    class FakeMaster:
        def get_stream(self, table_name: str) -> dict[str, object]:
            return {
                "stream_name": table_name,
    ...
    with pytest.raises(TimeoutError, match="timed out waiting for table source=\\[ghost_ticks\\]"):
        zippy._wait_for_table_ready("ghost_ticks", FakeMaster(), "20ms")
...
def test_subscribe_latency_probe_summarizes_latency_samples() -> None:
    ...
    minimal_values, minimal_rendered = module._callback_row_values(
        zippy.Row({"seq": 1, "localtime_ns": 123}),
        "minimal",
    )
    formatted_values, formatted_rendered = module._callback_row_values(
        zippy.Row(
            {
                "seq": 2,
                "instrument_id": "IF2606",
                "dt": "2026-04-02T09:30:00Z",
                "localtime_ns": 456,
                "last_price": 4100.2,
            }
        ),
        ...
        [
            {
                "seq": 1,
                "latency_ms": 0.2,
                "callback_enter_minus_append_ms": 0.1,
                "callback_work_ms": 0.01,
                "callback_done_delay_ms": 0.21,
                "rollover_first_row": False,
            },
            {
                "seq": 2,
                "latency_ms": 3.0,
                "callback_enter_minus_append_ms": 2.8,
                "callback_work_ms": 0.02,
                "callback_done_delay_ms": 3.02,
                "rollover_first_row": False,
            },
            {
                "seq": 3,
                "latency_ms": 1.4,
                "callback_enter_minus_append_ms": 1.1,
                "callback_work_ms": 0.03,
                "callback_done_delay_ms": 1.43,
                "rollover_first_row": True,
            },
        ],
        limit=2,
    )
    ...
    assert minimal_values == {"seq": 1, "localtime_ns": 123}
    assert minimal_rendered is None
    assert formatted_values["seq"] == 2
    assert formatted_rendered == (
        "seq=2 instrument_id=IF2606 dt=2026-04-02T09:30:00Z " "localtime_ns=456 last_price=4100.2"
    )
    assert slowest_rows == [
        {
            "seq": 2,
...
def test_remote_gateway_perf_package_helper_summarizes_delivery_metrics() -> None:
    import zippy.remote_gateway_perf as remote_gateway_perf

    report = remote_gateway_perf.build_subscribe_perf_report(
        uri="zippy://127.0.0.1:17690/default",
        stream="qmt_ticks",
        expected_rows=3,
        batch_size=2,
        ...
        first_batch_wait_ms=0.5,
        elapsed_ms=1.5,
        gateway_metrics_before={
            "subscribe_tables_delivered_total": 20,
            "subscribe_table_rows_delivered_total": 200,
        },
        gateway_metrics_after={
            "subscribe_tables_delivered_total": 22,
            "subscribe_table_rows_delivered_total": 203,
        },
    )

    assert report["received_tables"] == 2
    assert report["received_rows"] == 3
    assert report["gateway_delivery_delta"]["subscribe_tables_delivered_total"] == 2
    assert report["gateway_delivery_delta"]["subscribe_table_rows_delivered_total"] == 3
    assert report["gateway_delivery_matches_received"] is True
...
def test_subscribe_latency_probe_runs_against_temp_master(tmp_path: Path) -> None:
    ...
    assert report["callback_work"] == "minimal"
    assert report["measured_rows"] == 19
    assert report["append_latency_ms"]["count"] == 20
    assert report["rollover_append_latency_ms"]["count"] == 2
    assert report["callback_enter_delay_ms"]["count"] == 19
    assert report["callback_enter_minus_append_ms"]["count"] == 19
    assert report["callback_work_ms"]["count"] == 19
    assert report["callback_done_delay_ms"]["count"] == 19
    assert report["latency_ms"]["count"] == 19
    assert len(report["slowest_rows"]) == 3
    assert "callback_enter_minus_append_ms" in report["slowest_rows"][0]
    assert "callback_work_ms" in report["slowest_rows"][0]
    assert "callback_done_delay_ms" in report["slowest_rows"][0]
...
def test_master_client_publishes_and_gets_segment_descriptor(tmp_path: Path) -> None:
    ...
    descriptor["writer_epoch"] = client.get_stream("openctp_ticks")["writer_epoch"]
...
def test_query_tail_reads_latest_available_active_segment_rows(tmp_path: Path) -> None:
    ...
    writer = _segment_test_writer(client, "openctp_ticks", tick_schema, row_capacity=16)
...
def test_query_scan_live_returns_reader_over_retained_sealed_and_active_rows() -> None:
    ...
    with tempfile.TemporaryDirectory(dir="/tmp") as directory:
        ...
        try:
            ...
            for _ in range(50):
                stream = zippy._DEFAULT_MASTER.get_stream("openctp_ticks")
                sealed_segments = stream.get("sealed_segments") or []
                if stream.get("descriptor_generation", 0) >= 4 and len(sealed_segments) == 3:
                    break
                time.sleep(0.02)
...
def test_stream_table_e2e_writer_expiration_marks_stream_stale_and_blocks_live_reads(
    ...
    try:
        ...
        writer = _segment_test_writer(writer_client, "stale_ticks", schema, row_capacity=16)
        writer.append_tick(1777017600000000000, "IF2606", 4102.5)
...
def test_query_snapshot_includes_published_files() -> None:
    ...
    with tempfile.TemporaryDirectory(dir="/tmp") as directory:
        ...
        server = zippy.MasterServer(
            ...
            config={"table": {"persist": {"data_dir": str(root / "data")}}},
        )
        try:
            server.start()
        except RuntimeError as error:
...
def test_heartbeat_connection_loss_runs_hooks_without_unregistering_process() -> None:
    ...
    class FakeMasterClient:
        def heartbeat(self) -> None:
            raise RuntimeError("failed to connect to zippy master uri=[/tmp/zippy-master.sock]")
...
def test_master_client_replace_persisted_files_updates_stream_metadata(
    ...
    try:
        client = zippy.MasterClient(control_endpoint=control_endpoint)
        schema = pa.schema([("instrument_id", pa.string())])
        client.register_process("replace_persisted_files_test")
...
def _start_native_gateway_stack(
    *,
    ...
    gateway_endpoint = gateway_endpoint or f"127.0.0.1:{reserve_tcp_port()}"
    ...
    gateway.start()
    return server, gateway, master_uri, gateway.endpoint
...
def test_remote_gateway_writer_batches_rows_to_gateway() -> None:
    ...
    try:
        ...
        assert gateway.metrics()["written_rows_total"] == 0
...
def test_remote_gateway_writer_close_detaches_master_writer() -> None:
    ...
    try:
        ...
        writer.write({"instrument_id": "IF2606", "last_price": 4102.5})
        before_close = master.get_stream("qmt_close_ticks")
        assert before_close["status"] == "writer_attached"
        assert before_close["active_segment_descriptor"] is not None

        writer.close()

        after_close = master.get_stream("qmt_close_ticks")
        assert after_close["status"] == "registered"
        assert after_close["writer_process_id"] is None
        assert after_close["active_segment_descriptor"] is None
    finally:
        gateway.stop()
        server.stop()
        server.join()
...
def test_gateway_server_stop_detaches_open_master_writers() -> None:
    schema = pa.schema(
        [
            ("instrument_id", pa.string()),
            ("last_price", pa.float64()),
    ...
    try:
        ...
        writer.write({"instrument_id": "IF2606", "last_price": 4102.5})
        before_stop = master.get_stream("qmt_gateway_stop_ticks")
        assert before_stop["status"] == "writer_attached"
        assert before_stop["active_segment_descriptor"] is not None

        gateway.stop()

        after_stop = master.get_stream("qmt_gateway_stop_ticks")
        assert after_stop["status"] == "registered"
        assert after_stop["writer_process_id"] is None
        assert after_stop["active_segment_descriptor"] is None
    finally:
        gateway.stop()
        server.stop()
        server.join()
...
def test_gateway_server_requires_token_and_reports_metrics() -> None:
    ...
    try:
        ...
        table = zippy.read_table(
            "qmt_ticks",
            master=zippy.RemoteMasterClient(gateway_endpoint, token="secret-token"),
        ).collect()
        assert table.to_pydict() == {
            "instrument_id": ["IF2606"],
            "last_price": [4102.5],
...
def test_remote_read_table_collect_sends_plan_to_gateway() -> None:
    ...
    try:
        ...
        writer.write({"instrument_id": "IF2606", "last_price": 4102.5, "volume": 12})
        ...
        assert gateway.metrics()["collect_requests_total"] == 1
    finally:
        gateway.stop()
        server.stop()
        server.join()
...
def test_remote_subscribe_table_filters_at_gateway() -> None:
    schema = pa.schema(
        [
            ("instrument_id", pa.string()),
            ("last_price", pa.float64()),
    ...
    try:
        ...
        try:
            ...
            writer.write({"instrument_id": "IF2605", "last_price": 4101.5})
            writer.write({"instrument_id": "IF2606", "last_price": 4102.5})
            for _ in range(50):
                if received:
                    break
...
def test_remote_subscribe_filters_rows_at_gateway() -> None:
    schema = pa.schema(
        [
            ("instrument_id", pa.string()),
            ("last_price", pa.float64()),
...
def test_remote_subscribe_rejects_non_instrument_filter_before_gateway_request(
    monkeypatch,
) -> None:
    class FakeMasterClient:
        def get_config(self) -> dict[str, object]:
    ...
    def fail_remote_subscriber(*args, **kwargs):
        raise AssertionError("remote subscriber must not be created for unsupported row filter")

    monkeypatch.setattr(zippy, "RemoteStreamSubscriber", fail_remote_subscriber)

    with pytest.raises(
        ValueError,
        match="subscribe row filter only supports zp.col\\('instrument_id'\\) equality/is_in",
    ):
        zippy.subscribe(
            "qmt_ticks",
            callback=lambda row: None,
...
def test_remote_subscribe_table_rejects_complex_filter_before_gateway_request(
    monkeypatch,
) -> None:
    class FakeMasterClient:
        def get_config(self) -> dict[str, object]:
    ...
    def fail_remote_subscriber(*args, **kwargs):
        raise AssertionError("remote subscriber must not be created for unsupported table filter")

    monkeypatch.setattr(zippy, "RemoteStreamSubscriber", fail_remote_subscriber)

    with pytest.raises(
        ValueError,
        match="subscribe_table filter only supports simple zp.col predicates",
    ):
        zippy.subscribe_table(
            "qmt_ticks",
            callback=lambda table: None,
            master=FakeMasterClient(),
            filter=(zippy.col("last_price") + 1.0) > 4000.0,
        )
...
def test_remote_gateway_e2e_through_tcp_master_config() -> None:
    ...
    try:
        ...
        writer.close()
        stream = zippy.MasterClient(uri=master_uri).get_stream("remote_e2e_ticks")
        assert stream["status"] == "registered"
        assert stream["writer_process_id"] is None
        assert stream["active_segment_descriptor"] is None
...
def _allow_fake_persisted_paths(monkeypatch) -> None:
    monkeypatch.setattr(
        zippy,
        "_validated_persisted_file_path",
        lambda item: str(item["file_path"]),
    )
...
def test_validated_persisted_file_path_accepts_file_inside_root(tmp_path: Path) -> None:
    root = tmp_path / "persisted"
    root.mkdir()
    parquet_file = root / "ticks.parquet"
    parquet_file.write_bytes(b"not really parquet")

    assert zippy._validated_persisted_file_path(
        {
            "file_path": str(parquet_file),
...
def test_validated_persisted_file_path_rejects_file_outside_root(tmp_path: Path) -> None:
    root = tmp_path / "persisted"
    outside = tmp_path / "outside"
    root.mkdir()
    outside.mkdir()
    parquet_file = outside / "ticks.parquet"
    parquet_file.write_bytes(b"not really parquet")

    with pytest.raises(RuntimeError, match="outside persist data root"):
        zippy._validated_persisted_file_path(
            {
                "file_path": str(parquet_file),
                "persist_data_root": str(root),
            }
...
def test_validated_persisted_file_path_rejects_missing_root(tmp_path: Path) -> None:
    parquet_file = tmp_path / "ticks.parquet"
    parquet_file.write_bytes(b"not really parquet")

    with pytest.raises(RuntimeError, match="persist_data_root"):
        zippy._validated_persisted_file_path({"file_path": str(parquet_file)})


def test_scan_persisted_rejects_metadata_without_root(monkeypatch, tmp_path: Path) -> None:
    schema = pa.schema([("instrument_id", pa.string())])
    parquet_file = tmp_path / "ticks.parquet"
    pq.write_table(pa.table({"instrument_id": ["IF2606"]}, schema=schema), parquet_file)
    ...
    class FakeNativeQuery:
        ...
        def stream_info(self) -> dict[str, object]:
            return {
                "stream_name": self.source,
                "persisted_files": [{"file_path": str(parquet_file), "row_count": 1}],
            }

    monkeypatch.setattr(zippy, "_NativeQuery", FakeNativeQuery)

    query = zippy.read_table("ctp_ticks", master=object())

    with pytest.raises(RuntimeError, match="persist_data_root"):
        query._scan_persisted()


def test_compact_table_rejects_source_metadata_without_root(tmp_path: Path) -> None:
    schema = pa.schema([("instrument_id", pa.string())])
    first_file = tmp_path / "part-000001.parquet"
    second_file = tmp_path / "part-000002.parquet"
    pq.write_table(pa.table({"instrument_id": ["IF2606"]}, schema=schema), first_file)
    pq.write_table(pa.table({"instrument_id": ["IF2607"]}, schema=schema), second_file)
    ...
    class FakeMaster:
        ...
        def replace_persisted_files(
            ...
            raise AssertionError("unsafe metadata must be rejected before replacement")

    with pytest.raises(RuntimeError, match="persist_data_root"):
        zippy.ops.compact_table("ctp_ticks", master=FakeMaster())
...
def test_remote_explain_reports_gateway_residual_plan(monkeypatch) -> None:
    schema = pa.schema(
        [
            ("instrument_id", pa.string()),
            ("seq", pa.int64()),
    ...
    class FakeRemoteQuery:
        ...
        def collect_plan(self, plan: list[dict[str, object]], *, snapshot: bool) -> pa.Table:
            return pa.table({"instrument_id": [], "seq": []}, schema=schema)
    ...
    explain = (
        zippy.read_table("ctp_ticks", master=FakeMaster(), snapshot=False)
        .head(2)
        .filter(zippy.col("instrument_id") == "IF2606")
        .explain()
    )

    assert explain["executor"] == "remote"
    assert [item["op"] for item in explain["gateway_plan"]] == ["head"]
    assert [item["op"] for item in explain["gateway_residual_plan"]] == ["filter"]
    assert explain["capability"]["gateway_plan_ops"] == ["head"]
    assert explain["capability"]["gateway_residual_ops"] == ["filter"]
    assert explain["capability"]["unsupported_gateway_ops"] == []


def test_remote_explain_reports_join_as_gateway_residual_plan(monkeypatch) -> None:
    schema = pa.schema(
        [
            ("instrument_id", pa.string()),
            ("last_price", pa.float64()),
    ...
    master = FakeMaster()
    query = zippy.read_table("future_ticks", master=master).join(
        zippy.read_table("spot_ticks", master=master),
        on="instrument_id",
    )
    ...
    assert explain["executor"] == "remote"
    assert [item["op"] for item in explain["gateway_plan"]] == []
    assert explain["gateway_residual_plan"] == [
        {
            "op": "join",
            "right_source": "spot_ticks",
            "on": ["instrument_id"],
            "how": "inner",
            "suffix": "_right",
        }
    ]
    assert explain["capability"]["gateway_plan_ops"] == []
    assert explain["capability"]["gateway_residual_ops"] == ["join"]
    assert explain["capability"]["unsupported_gateway_ops"] == ["join"]
...
def test_remote_collect_reuses_pinned_snapshot(monkeypatch) -> None:
    ...
    class FakeRemoteQuery:
        ...
        def snapshot(self) -> dict[str, object]:
            snapshots_created.append(self.source)
            return {
                "stream_name": self.source,
        ...
        def collect_plan(self, plan: list[dict[str, object]], *, snapshot: object) -> pa.Table:
            collect_snapshots.append(snapshot)
            return pa.table({"instrument_id": ["IF2606"], "seq": [1]}, schema=schema)
    ...
    query = zippy.read_table("ctp_ticks", master=FakeMaster(), snapshot=True)
    query.collect()
    query.collect()

    assert snapshots_created == ["ctp_ticks"]
    assert collect_snapshots == [
        {
            "stream_name": "ctp_ticks",
            "data_path": "remote_gateway",
            "snapshot_id": "snapshot-1",
...
def test_remote_query_sends_snapshot_id_and_releases_snapshot(monkeypatch) -> None:
    schema = pa.schema([("instrument_id", pa.string())])
    ...
    def fake_remote_request(
        ...
        *,
        ...
        if header["kind"] == "create_snapshot":
            return (
                {
                    "status": "ok",
                    "snapshot": {
        ...
        if header["kind"] == "collect":
            return {"status": "ok", "metrics": {}}, payload
        if header["kind"] == "release_snapshot":
            return {"status": "ok"}, b""
        raise AssertionError(f"unexpected request {header}")
    ...
    snapshot = remote_query.snapshot()
    result = remote_query.collect_plan([], snapshot=snapshot)
    remote_query.release_snapshot("snapshot-1")
...
def test_remote_collect_applies_filter_after_head_locally(monkeypatch) -> None:
    ...
    source_table = pa.table(
        {
            "instrument_id": ["IF2606", "IF2607", "IF2606"],
            "seq": [1, 2, 3],
        },
        schema=schema,
    )
    ...
    class FakeRemoteQuery:
        ...
        def collect_plan(self, plan: list[dict[str, object]], *, snapshot: bool) -> pa.Table:
            ...
            for item in plan:
                if item["op"] == "head":
                    result = result.slice(0, int(item["n"]))
            return result
    ...
    assert [item["op"] for item in sent_plans[0]] == ["head"]
    assert result.to_pydict() == {"instrument_id": ["IF2606"], "seq": [1]}


def test_remote_collect_applies_filter_after_slice_locally(monkeypatch) -> None:
    schema = pa.schema(
        [
            ("instrument_id", pa.string()),
            ("seq", pa.int64()),
    ...
    source_table = pa.table(
        {
            "instrument_id": ["IF2606", "IF2607", "IF2606", "IF2608"],
            "seq": [1, 2, 3, 4],
        },
        schema=schema,
    )
    ...
    assert [item["op"] for item in sent_plans[0]] == ["slice"]
    assert result.to_pydict() == {"instrument_id": ["IF2606"], "seq": [3]}
...
def test_connect_sets_default_master_for_query_tail(tmp_path: Path) -> None:
    ...
    table = zippy.read_table("openctp_ticks")
    latest = table.tail(10).collect()
    ...
    assert info["write_seq"] == 0
    assert info["writer_epoch"] >= 1
    ...
    assert client.get_stream("openctp_ticks")["segment_reader_leases"]
...
def test_subscribe_resumes_after_writer_process_restart(tmp_path: Path) -> None:
    ...
    writer = _segment_test_writer(writer_client, "restart_ticks", tick_schema, row_capacity=16)
...
def test_subscriber_metrics_expose_hybrid_mmap_wait_counters(tmp_path: Path) -> None:
    ...
    writer = zippy._internal._SegmentTestWriter(
        ...
        tick_schema,
        row_capacity=16,
        writer_epoch=client.get_stream("hybrid_wait_ticks")["writer_epoch"],
    )
...
def test_subscribe_switches_from_sealed_segment_when_descriptor_arrives(
    ...
    writer = zippy._internal._SegmentTestWriter(
        ...
        tick_schema,
        row_capacity=4,
        writer_epoch=client.get_stream("rollover_ticks")["writer_epoch"],
    )
...
def test_reactive_latest_named_segment_source_starts_from_live_tail(tmp_path: Path) -> None:
    ...
    writer = _segment_test_writer(writer_client, "openctp_ticks", tick_schema, row_capacity=16)
    writer.append_tick(1777017600000000000, "IF2606", 4102.5)
    writer.append_tick(1777017601000000000, "IF2606", 4103.0)
    writer.append_tick(1777017602000000000, "IF2607", 4104.5)
...
def test_stream_table_source_stop_can_reenter_engine_status() -> None:
    ...
    class ReentrantStopHandle:
        ...
        def stop(self) -> None:
            ...
            self.owner.status_seen_during_stop = self.owner.engine.status()
            self.stopped = True

        def join(self) -> None:
            return None

    class ReentrantStopSource:
        def __init__(self) -> None:
            self.engine = None
            self.handle: ReentrantStopHandle | None = None
            self.status_seen_during_stop: str | None = None

        def _zippy_output_schema(self) -> pa.Schema:
            return tick_schema

        def _zippy_start(self, sink) -> ReentrantStopHandle:
            sink.emit_hello("reentrant_stop_ticks")
            self.handle = ReentrantStopHandle(self)
            return self.handle

    source = ReentrantStopSource()
    engine = zippy._internal.StreamTableMaterializer(
        name="reentrant_stop_ticks",
        input_schema=tick_schema,
        source=source,
        target=zippy.NullPublisher(),
    )
    ...
    assert source.handle is not None
    assert source.handle.stopped
    assert source.status_seen_during_stop in {"running", "stopping", "stopped"}
    assert engine.status() == "stopped"
...
def test_pipeline_stream_table_persist_publishes_master_metadata(
    ...
    assert kwargs["descriptor_forwarding"] is True
...
def test_pipeline_append_table_publish_uses_stream_table_options(
    monkeypatch,
    tmp_path: Path,
) -> None:
    tick_schema = pa.schema([("instrument_id", pa.string())])
    ...
    class FakeMaster:
        ...
        def register_stream(
            ...
            created["stream"] = (stream_name, schema, buffer_size, frame_size)
        ...
        def register_source(
            ...
            created["source"] = (source_name, source_type, output_stream, config)

        def get_stream(self, stream_name: str) -> dict[str, object]:
            return {"writer_epoch": 7, "segment_reader_leases": []}

        def publish_segment_descriptor(self, stream_name: str, descriptor: object) -> None:
            created["descriptor"] = (stream_name, descriptor)
    ...
    monkeypatch.setattr(zippy, "_StreamTableMaterializer", CapturingMaterializer)

    pipeline = (
        zippy.Pipeline("test_ingest", master=FakeMaster())
        .append_table(
            "openctp_ticks",
            schema=tick_schema,
            buffer_size=128,
    ...
    assert pipeline._stream_name == "openctp_ticks"
    assert created["stream"] == ("openctp_ticks", tick_schema, 128, 8192)
    assert created["source"][2] == "openctp_ticks"
    assert created["kwargs"]["row_capacity"] == 64
    assert created["kwargs"]["persist_path"] == str(tmp_path / "archive" / "openctp_ticks")
    assert created["kwargs"]["descriptor_forwarding"] is True


def test_pipeline_stream_table_persist_keeps_descriptor_forwarding_enabled(
    monkeypatch,
    tmp_path: Path,
) -> None:
    tick_schema = pa.schema([("instrument_id", pa.string())])
    ...
    assert created["kwargs"]["persist_path"] == str(tmp_path / "ctp_ticks")
    assert callable(created["kwargs"]["persist_publisher"])
    assert created["kwargs"]["descriptor_forwarding"] is True


def test_pipeline_stream_table_persist_falls_back_to_live_persist_with_retention(
    monkeypatch,
    tmp_path: Path,
) -> None:
    tick_schema = pa.schema([("instrument_id", pa.string())])
...
def test_pipeline_stream_table_persist_writes_parquet_and_master_metadata(
    tmp_path: Path,
) -> None:
    server, control_endpoint = start_master_server(tmp_path)
    tick_schema = pa.schema(
    ...
    persist_path = tmp_path / "persist_ticks"
    master = zippy.MasterClient(control_endpoint=control_endpoint)
    pipeline = zippy.Pipeline("persist_pipeline", master=master).stream_table(
        "persist_ticks",
        schema=tick_schema,
        row_capacity=2,
        persist_path=zippy.ParquetPersist(persist_path),
    )
    ...
    try:
        ...
        stream = master.get_stream("persist_ticks")
        assert len(stream["persisted_files"]) == 2
        assert {item["writer_epoch"] for item in stream["persisted_files"]} == {
            stream["writer_epoch"]
        }
    finally:
        server.stop()
...
def test_master_client_registers_control_plane_entities_and_updates_status(
    ...
    with pytest.raises(TypeError, match="register_sink is no longer supported"):
