...
impl LayoutPlan {
    ...
    pub fn total_bytes(&self) -> usize {
        self.columns
            .iter()
            .map(|column| {
                let validity_end = column.validity_offset + column.validity_len;
                let offsets_end = column.offsets_offset + column.offsets_len;
                let values_end = column.values_offset + column.values_len;
                validity_end.max(offsets_end).max(values_end)
            })
            .max()
            .unwrap_or(0)
    }
