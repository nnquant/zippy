...
fn spawn_test_runtime_with_behavior(
    ...
    let handle = spawn_source_engine_with_publisher(
        Box::new(source),
        engine,
        test_engine_config(overflow_policy),
        NoopPublisher,
    )
    .unwrap();
...
fn spawn_test_pipeline_runtime() -> (TestPipelineRuntime, Sender<()>) {
    spawn_test_runtime_with_behavior(SourceMode::Pipeline, OverflowPolicy::Block, false)
}

fn spawn_test_pipeline_runtime_with_behavior(
    fail_after_first_release: bool,
) -> (TestPipelineRuntime, Sender<()>) {
    spawn_test_runtime_with_behavior(SourceMode::Pipeline, OverflowPolicy::Block, fail_after_first_release)
}

fn spawn_test_consumer_runtime() -> (TestPipelineRuntime, Sender<()>) {
    spawn_test_runtime_with_behavior(SourceMode::Consumer, OverflowPolicy::Block, false)
}

fn test_engine_config(overflow_policy: OverflowPolicy) -> EngineConfig {
    EngineConfig {
