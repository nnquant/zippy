...
fn build_test_span(start: usize, end: usize) -> Result<zippy_segment_store::RowSpanView, String> {
    ...
    let layout =
        zippy_segment_store::LayoutPlan::for_schema(&schema, end).map_err(str::to_owned)?;
    let mut writer = zippy_segment_store::ActiveSegmentWriter::new_for_test(schema, layout)
        .map_err(str::to_owned)?;
