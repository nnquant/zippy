...
- 输出标的列以及标准 K 线列：
  `instrument/dt/open/high/low/close/volume/total_turnover/num_trades/limit_up/limit_down/start_dt/close_dt/end_dt`。
- 支持输入缺少 `num_trades`、`limit_up`、`limit_down` 字段时输出对应 nullable 列。
- 丢弃非交易时段无效 tick，不生成 K 线，也不污染累计成交基线。
- 支持集合竞价归属策略。
- 支持 `dt` 标签策略，在窗口起点和窗口终点之间选择主时间标签。
...
num_trades, limit_up, limit_down, start_dt, close_dt, end_dt
```

其中：

- `instrument` 使用输入标的列的值和类型，v1 要求为 `utf8`。
- `dt/start_dt/close_dt/end_dt` 为 timezone-aware nanosecond timestamp，时区来自 profile。
