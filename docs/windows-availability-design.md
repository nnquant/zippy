...
Windows 原生 runtime 可用性作为独立专题推进，不挂在 Linux 低延迟 IPC 主线之下，也不占用
主 ROADMAP 中的 `M7 Segment-native Downstream Engine Composition` 编号。低延迟 IPC 的抽象
可以被 Windows 复用，但 Windows 的核心目标是跨平台 runtime 可用性和原生平台验证。

### WN1 Windows compile baseline
...
### WN2 Cross-platform control transport
...
### WN3 Cross-platform segment mapping
...
### WN4 Cross-platform segment notification
...
推荐作为独立 `Windows Native Runtime` 专题推进，阶段顺序为
`WN1 -> WN2 -> WN3 -> WN4 -> WN5`。它与
`docs/cross-platform-remote-gateway-design.md` 的关系是：

```text
Remote Gateway：Windows client -> WSL/Linux Zippy core，解决近期 QMT/Windows 客户端接入；
Windows Native Runtime：Windows 本地 master + Windows segment backend，解决长期原生运行。
```
