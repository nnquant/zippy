...
Windows 可用性作为独立里程碑推进，不挂在 Linux 低延迟 IPC 主线之下。低延迟 IPC 的抽象可以被
Windows 复用，但 Windows 的核心目标是跨平台 runtime 可用性和原生平台验证。

### M7.1 Windows compile baseline
...
### M7.2 Cross-platform control transport
...
### M7.3 Cross-platform segment mapping
...
### M7.4 Cross-platform segment notification
...
### M7.5 Windows e2e and packaging
...
推荐作为独立 `M7 Cross-platform Runtime` 推进，阶段顺序为
